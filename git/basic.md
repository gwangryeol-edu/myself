...
# git reset
## git reset 이란 어떤 명령어 인가
- 원하는 커밋 시점으로 되돌리기 위한 명령어
## git reset의 3가지 옵션은 무엇이고, 각각의 의미는 무엇인가
- `--soft`: 커밋 취소o | 스테이지 영역 취소x | 파일 삭제x
- `--mix` : 커밋 취소o | 스테이지 영역 취소o | 파일 삭제x
- `--hord`: 커밋 취소o | 스테이지 영역 취소o | 파일 삭제o

## 기억에 남아있는 git에 대한 내용을 모두 작성

### 스냅샷
- 사진을 찍은 듯 작업물을 기록하는 행위
### 지역 저장소, 원격 저장소
- 지역 저장소: 특정 pc에서 저장되어 있어 다른 pc에서 접근이 곤란
- 원격 저장소: 서버에 저장되어 있어 다른 pc로도 작업물이나 작업 환경 등을 사용 가능
### git
- 작업물을 스냅샷을 이용하여 기록하고 관리하는 프로그램
